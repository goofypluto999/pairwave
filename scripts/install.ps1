# Pairwave one-line installer (Windows PowerShell 5.1+ / pwsh). ASCII only: PS 5.1 reads BOM-less
# files as ANSI, so any smart punctuation would corrupt the parser.
#
...
#
# Does exactly this: checks git+node, clones/updates Pairwave into %LOCALAPPDATA%\pairwave\app,
# builds it, installs a global `pairwave` command, then wires THE FOLDER YOU RAN IT FROM
# (room config + Claude Code MCP entry + /pairwave skill) and prints next steps.
...
# 3. build (output suppressed unless something fails)
Step "building (first run takes about 30s)..."
Push-Location $app
try {
  ...
  # Some machines (AV / folder-protection policies) break npm's workspace junctions. Probe one;
  # if it does not resolve, replace the links with real copies - node treats them the same.
...
# 4. global pairwave command (shim in npm's global bin, which is already on your PATH)
